#!/usr/bin/env node
//
// Installs the OpenClaw proactive-poll scheduled task for the `personal` group (Nova).
//
...
// AND it is outside quiet hours. On wake, Nova summarizes the items into ONE
...
You are Nova. This is an automated proactive sweep of Steph's inbox/tasks/routines/shift. Ignore the stale count in the <script_output> block above — always re-fetch for the authoritative current list.

<output_contract>
Every path below, whether you post or stop: your final turn reply is auto-forwarded to Discord, so it must be only an <internal>…</internal> block with nothing before or after it (e.g. <internal>posted 4 items, acked</internal> or <internal>nothing pending, stopped</internal>). The brief reaches Steph only through the send_message tool — never as your final reply; any text outside <internal> tags is posted to him as a separate message.
</output_contract>
...
2) Stop conditions:
   - If the call errors or returns non-200 (e.g. a bad/expired secret): treat it as a transient failure — don't post, don't ack, end with your <internal> status only. It retries next tick. Never read an error as "nothing pending" (that would silence a real backlog).
   - If it returns 200 but items is empty (count 0): also stop — no send_message, no ack, end with your <internal> status only (the items were handled since the pre-check). Don't send an empty or "nothing to report" message.

3) Otherwise compose one concise Discord message and deliver it via the send_message tool (not as your final reply):
   - Keep it under ~1800 characters. Discord hard-splits anything over 2000 chars mid-line into garbled posts — if there are more items than fit, lead with the most important and summarize the rest as counts (e.g. "+5 more needs-reply emails").
   ...
   - Short bullets built from each item's title/body — don't invent details.
   ...
   - Open with the canonical header (📋 Inbox Check-in — <weekday>, <Mon DD> taken from the [DATE CONTEXT] line) and end with the obvious next action. Example:
   <example>
   📋 Inbox Check-in — Thursday, Jun 18

   *Overdue / today*
   • Reply to Chief Martin re: Saturday drill (waiting since Tue)
   • Task due today: submit FR order #4412

   *Needs reply* (+5 more emails)
   • ⚖️ Lise Danis — disclosure deadline question

   Want me to draft replies, or mark the order done?
   </example>
...
5) Only after send_message returns, ack exactly the items you surfaced so they stop reappearing:
   ...
   Ack only the items you actually included — re-acking a re-surfaced item refreshes its ~2-day timer; if you post it but don't ack, it keeps nagging every tick.

6) Order matters: send_message first, then write .last-nudge.json, then ack. If send_message errors, don't write the file or ack (the item re-surfaces next run instead of being lost). If the ack call errors, that's fine — the item simply reappears next time.

<side_effects>Your only side effects are send_message, the single .last-nudge.json write, and the ack call — nothing else (no scheduling, no other file writes). End with your <internal> status only.</side_effects>`;
